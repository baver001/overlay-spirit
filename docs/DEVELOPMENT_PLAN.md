...
### Этап 1. Подготовка к админ-панели (недели 3–4) ✅ ЗАВЕРШЁН
- ✅ **База данных**: доработан `schema.sql` (индексы по датам покупок, таблицы `app_settings`, `admin_audit`, добавлено поле `discount_price_cents`).
- ✅ **API слой**: расширен `functions/api/admin.ts` — фильтры, статистика, настройки, аудит, file upload handler.
- ✅ **UI-фрейм**: прототип layout `/admin` (Sidebar + Header), роутинг, auth guard через `AdminSessionProvider`.
- ✅ **R2 загрузки**: валидация MIME/размера (только JPG, макс 10MB), структура ключей (`overlays/{nanoid}.jpg`), FormData handler.
- ✅ **Типы и клиенты**: созданы TS-интерфейсы для категорий, наборов, оверлеев, пользователей, покупок, настроек.

### Этап 2. Реализация админ-панели (недели 5–7) ✅ ЗАВЕРШЁН
- ✅ **Стартовая версия**: реализован модуль `/admin` со следующими разделами: дашборд, категории, наборы, пользователи, покупки, настройки.
- ✅ **Статистика**: построение графиков покупок и выручки (Recharts), агрегированные метрики, топ-наборы и журнал действий из таблицы `admin_audit`.
- ✅ **Журнал действий**: добавлена таблица `admin_audit`, запись операций настроек, список последних действия в админке.
- ✅ **CRUD категории**: полный цикл создания/редактирования/удаления категорий с автоматической генерацией slug, drag&drop для изменения порядка.
- ✅ **CRUD наборы**: формы создания/редактирования наборов с загрузкой JPG-файлов, выбором категории из списка, настройкой цен и скидок.
- ✅ **Загрузчик ассетов**: FormData upload для JPG файлов в R2 с правильными ключами (`overlays/{nanoid}.jpg`).
- ✅ **Настройки Stripe**: интерфейс для ввода API ключей, webhook secret, страны аккаунта.
- ✅ **UI polishing**: подтверждение удаления, оптимизация запросов (React Query), уведомления (toast), accessibility (DialogDescription).
- ✅ **Исправления безопасности**: замена crypto.randomUUID на nanoid, добавление ON DELETE SET NULL для category_id, улучшенная обработка ошибок.
- ✅ **Интеграция витрины**: публичный каталог получает данные из D1 с превью оверлеев, файлы доступны через `/api/files/`.
